...
namespace test
{
        ...
        m_CamSpeed(12.f)
    {
        ...
        m_WorldCenter = m_Scene->CreateEntity("WorldCenter");
        ...
        for (int i = 0; i < num; ++i)
        {
            for (int j = 0; j < num; ++j)
            {
                for (int k = 0; k < num; ++k)
                {
                    m_Cubes[i*num*num + j*num + k] = m_Scene->CreateEntity("Cube" + std::to_string(i));
                    m_Cubes[i * num * num + j * num + k].AddComponent<ModelComponent>(m_CubeMesh);
                    m_Cubes[i * num * num + j*num + k].GetComponent<TransformComponent>().
        ...
        for (int i = 0; i < 6; ++i)
        {
            m_Brickwalls[i] = m_Scene->CreateEntity("Brickwall" + std::to_string(i));
        ...
        m_LightCubes[0] = m_Scene->CreateEntity("LightCube");
